...
export default function AdminStudents({ search }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {showAdd && (
        <div className="bg-white border border-indigo-100 rounded-2xl p-6 shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
              <UserPlus className="w-4 h-4 text-indigo-500" /> Register New Student
            </h3>
          ...
          <form onSubmit={handleRegister} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input className={inp} placeholder="Student ID (e.g. 21A01)" required value={form.studentId} onChange={e => setForm({...form, studentId: e.target.value})} />
            ...
            <input className={inp} placeholder="Mobile Number" required value={form.mobile} onChange={e => setForm({...form, mobile: e.target.value})} />
            ...
            <div className="md:col-span-3 flex justify-end">
              <button type="submit" disabled={addLoading} className="bg-indigo-600 text-white px-6 py-2 rounded-lg text-sm font-bold hover:bg-indigo-700 disabled:opacity-50 transition-all">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="px-7 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Student Directory</span>
          </div>
          <div className="flex items-center gap-3">
            {!showAdd && (
              <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-3 py-1 bg-white border border-indigo-200 text-indigo-600 rounded-lg text-[11px] font-bold hover:bg-indigo-50 transition-all">
                <UserPlus className="w-3.5 h-3.5" /> Add Student
              </button>
            )}
            <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full">{students.length} Registered</span>
          </div>
        </div>

        {loading && <div className="text-center py-16 text-slate-400 text-sm">Loading students...</div>}
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  {['Student ID', 'Name', 'Branch', 'Year', 'Mobile', 'Actions'].map(h => (
              ...
              <tbody>
                {students.map(student => (
                  <tr key={student._id} className="border-b border-slate-100 last:border-none hover:bg-slate-50/60 transition-colors">
                    <td className="px-7 py-4 font-bold text-slate-700 text-sm">{student.studentId}</td>
                    <td className="px-7 py-4 font-medium text-indigo-600 text-sm">{student.name}</td>
                    <td className="px-7 py-4 text-slate-500 text-sm">{student.branch || 'N/A'}</td>
                    <td className="px-7 py-4 text-slate-500 text-sm">{student.year || 'N/A'}</td>
                    <td className="px-7 py-4 text-slate-600 text-sm">{student.mobile}</td>
                    <td className="px-7 py-4">
                      {student.role === 'student' && (
                        <button
                          onClick={() => handleMessage(student.studentId, student.name)}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 hover:text-indigo-700 rounded-lg text-xs font-bold transition-colors"
                        >
